// 이미지 데이터를 저장할 데이터베이스 스키마 정의
// 편지 보내기에 첨부한 이미지에 대한 정보를 저장하는 용도의 DB
...
const imageSchema = new Schema({
  ...
  post_id: {
    // 해당 이미지를 첨부한 편지의 id 값
    // 편지를 보여줄 때 이어진 이미지와 함께 보여줘야하기 때문에 스키마에 함께 저장한다.
    type: Number,
    required: true,
  },
});
...
imageSchema.plugin(autoIncrement.plugin, {
  // img_id에 대해 자동으로 숫자 카운트를 해주기 위해 사용
  model: 'Image',
  field: 'img_id',
...
module.exports = mongoose.model('Image', imageSchema); // 외부에서 해당 스키마를 사용할 수 있도록 설정
